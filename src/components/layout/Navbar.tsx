  ...
  if (isPageRoute) {
    return (
      ...
      >
        {label}
        <svg
          className="absolute bottom-px left-1/2 h-1 w-0 -translate-x-1/2 transition-all duration-300 group-hover:w-[70%]"
          viewBox="0 0 100 2"
        ...
        >
          ...
          <path
            d="M0 1 Q 50 2, 100 1 Q 50 0, 0 1 Z"
            fill={`url(#grad-${label})`}
          />
        </svg>
      </Link>
...
export default function Navbar() {
  ...
  return (
    <header className="relative z-50 h-[68px] w-full lg:h-[136px]">
      <div className="fixed inset-x-0 top-0 z-50 w-full">
        ...
        <div
          className={`hidden w-full overflow-hidden bg-[#151c29] transition-all duration-300 ease-in-out lg:block ${
            hideTopBar
              ...
              : "max-h-[64px] border-b border-[#292F40] opacity-100"
          }`}
        >
        ...
        {/* DESKTOP MAIN NAVBAR */}
        <div className="relative hidden w-full border-b border-[#FFD34D] bg-[#151c29] shadow-[0_6px_24px_rgba(0,0,0,0.35)] lg:block">
          ...
          <div className="mx-auto grid min-h-[72px] w-full max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-6 md:px-10">
            ...
            <div className="flex items-center justify-end gap-3 md:gap-5">
              <nav className="flex items-center gap-3 font-[var(--font-heading)] md:gap-5">
                {rightNav.map((item) => (
                  <NavLink
                    key={item.label}
                    label={item.label}
